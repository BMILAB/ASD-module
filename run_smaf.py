...
#maxItr=10
#activity_lower=0.
#module_lower=min(20,X.shape[0]/20)
#donorm=True
#mode=1
#use_chol=True
#mink=5
...
def smaf(X,d,lda1,lda2,maxItr=100,UW=None,posW=False,posU=True,use_chol=False,module_lower=500,activity_lower=5,donorm=False,mode=1,mink=5,U0=[],U0_delta=0.1,doprint=False):
	...
	for itr in range(maxItr):
		...
		if doprint:
			print(distance.correlation(X.values.flatten(),Xhat.flatten()),module_size,activity_size,lda1,lda2)
...
os.getcwd()
import pandas as pd
X = pd.read_csv(open("train.csv"),index_col=0)
#print(X)
#print(X.shape)
#print(X.values.flatten())
#print(X.columns) #colname
#print(X.index)  #rownames
#data = X.values #data.information
d = 500
k = 15
UW = (np.random.random((X.shape[0],d)),np.random.random((d,X.shape[1])))
U,W = smaf(X,d,k,0.1,maxItr=10,activity_lower=0.,module_lower=min(20,X.shape[0]/20),donorm=True,mode=1,use_chol=True,mink=5,doprint=True)

np.save("XXXXX_Umap.npy",U)
np.save("XXXXX_SMAF.npy",W)
#b = np.load("U_array.npy")

end_time = time.time()
runtime = end_time - start_time
print("Script running time", runtime, "sec")
...
#import pandas as pd
#X = pd.read_csv(open("./test.csv"),index_col=0)
#print(X.values.flatten())
#X= array(X)
#X.flatten
#
